...
export async function onRequestGet(context) {
  ...
  if (pathname === '/screeners') {
    ...
    try {
      const res = await fetch(`https://api.deltascreener.com/screens/public?page=${hubPage}&limit=24`, { signal: AbortSignal.timeout(6000) })
      if (res.ok) {
  ...
  if (pathname.startsWith('/screeners/')) {
    ...
    if (communityMatch) {
      ...
      try {
        const apiRes = await fetch(`https://api.deltascreener.com/screens/public/${communityMatch[1]}`, { signal: AbortSignal.timeout(6000) })
        if (apiRes.ok) {
    ...
    if (!screen) {
    ...
    } else {
      {
        ...
        try {
          ...
          const apiRes = await fetch('https://api.deltascreener.com/screener/custom', {
            method: 'POST',
