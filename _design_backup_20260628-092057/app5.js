    ...
    modal.innerHTML = `
      <div style="background:#141a26;border:1px solid rgba(255,255,255,.09);border-radius:16px;padding:32px 30px 28px;max-width:380px;width:92%;box-shadow:0 24px 64px rgba(0,0,0,.55);position:relative">
        ...
        <div style="display:flex;gap:8px">
          <input id="ds-pro-email-input" type="email" placeholder="you@email.com" style="flex:1;padding:10px 13px;border-radius:9px;border:1px solid rgba(255,255,255,.12);background:#0d1320;color:#f3f4f6;font-size:13.5px;outline:none" />
      ...
      <div class="container landing-hero-grid">
        <div class="landing-hero-copy landing-hero-minimal">
          ...
          <div class="hero-search landing-hero-search hero-search-prominent">
            ...
            <input type="text" id="hero-search" placeholder="Try: Apple, AAPL, Microsoft, NVDA…" autocomplete="off" autofocus />
...
function renderOverviewTab(c, ov, fins, rt) {
  ...
  const aboutFallback = `<p>${coName}${ov.exchange ? ` trades on ${ov.exchange}` : ''}${sectorLine ? ` in the ${sectorLine} space` : ''}. A detailed business description isn't available yet — the snapshot below summarizes what the financials show.</p>`
  c.innerHTML = `
      ...
      <div style="display:flex;flex-wrap:wrap;gap:24px;padding:8px 4px">
        ...
        <div>
          ...
          <div style="font-size:13px;color:#d1d5db;display:flex;align-items:center">
            ${fmt.compact((fins.annual.sales || []).filter(v=>v!=null).slice(-1)[0])}
        ...
        <div>
          ...
          <div style="font-size:13px;color:#d1d5db;display:flex;align-items:center">
            ${fmt.compact((fins.annual.netProfit || []).filter(v=>v!=null).slice(-1)[0])}
      ...
      <table class="tbl fin-tbl" data-fin-chartable="1">
        <thead>
          <tr>
            ...
            ${order.map(i => `<th>${headers[i]}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${rows.map((r, ri) => {
